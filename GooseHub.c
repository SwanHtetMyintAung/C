...
const char input_format[] = "state:%d,id:%d,token:%d,name:%[^,],email:%[^,],password:%[^,],trasition_record:";
const char output_format[] = "state:%d,id:%d,token:%d,name:%s,email:%s,password:%s,trasition_record:";
...
//utilities
int make_int_array(int* ptr, int user_id);
int get_biggest_int(int array[MAX_RECORD]);
int get_smallest_int(int array[MAX_RECORD] ,int initial);
int get_index_of_id(int id_array[MAX_RECORD],int record_id);
...
void dashboard(int id){
    ...
    while(1){
        ...
        if(is_admin == 1){
            ...
            printf("Enter 7 To Suspend A User(you can't delete other admin admin account)\n");
        }
        ...
        if(l_option == 1){
            ...
            clear_screen();
            if(database[g_is_login].suspend == 1){
                printf("You cannot Transfer Anypoint since you are being suspended!\n");
                continue;
            }
            ...
            if((check_admin(id_to_delete) != 1 ) && (id_to_delete != -1)){
                database[id_to_delete] = ResetUser ;
            }else{
                printf("can't delete the account for some reasons!\n");
            }
        }else if(l_option == 7 && is_admin == 1){
            clear_screen();
            int id_to_suspend = -1;
            printf("Enter The ID number you want to suspend:");
            scanf("%d",&id_to_suspend);
            //not 1 means it is not admin account
            if((check_admin(id_to_suspend) != 1) && (id_to_suspend != -1)){
                database[id_to_suspend].suspend = 1;
                printf("Suspended The Account Successfully!\n");
            }else{
                printf("Couldn't suspend the account for some reasons!\n");
            }
        }
    }
...
//utilities
//make integer array from trasition record Id
//still in progress
int make_int_array(int* ptr, int user_id){
    ...
    for(int i = 0; i < MAX_RECORD; i++){
        ptr[i] = database[user_id].transition_db[i].record_id;
    }
...
//find the index of the integer(id), but i'll be using the collective function 
//return the index of the id if found . return -1 if doesn't
int get_index_of_id(int id_array[MAX_RECORD],int record_id){
    //default -1 error code
    int the_index=-1;
    for(int i=0;i<MAX_RECORD;i++){
        if(id_array[i] == record_id){
            //if the looping is in the last index
            if(i == (MAX_RECORD-1)){
                the_index = 0;
                break;
            }else{
...
void save_transition_record(int token_amount , struct user *sender , struct user *receiver){
    ...
    t = time(NULL);
    struct tm* calendar;
    calendar = asctime(localtime(&t));
    //loop through the transition record of the sender to check which spots are available
    for(int i=0;i<MAX_RECORD;i++){
        ...
        if(first_cha == '\0'){
            ...
            //setting date
            sprintf(sender->transition_db[i].date,"%s",calendar);
            int size_of_data = size_of_array(sender->transition_db[i].date);
            //stripping \n from date
            sender->transition_db[i].date[size_of_data - 1] = '\0';
        ...
        //checking if the looping has reach maximum value which is MAX_RECORD , AND if data has not been written. the two condition basically means , the record database is full .
            ...
            int id_array[MAX_RECORD]= {0};
            //int *p_id_array = &id_array[0];
            make_int_array(&id_array,sender->id);
            int biggest_id = get_biggest_int(id_array);
            int id_to_write = get_index_of_id(id_array,biggest_id);
            if(id_to_write==-1){
                printf("Couldn't Save Record!");
                exit(-1);
            }
            //make the new into from the biggest id + 1
            sender->transition_db[id_to_write].record_id = (biggest_id+1);
            //set the date
            sprintf(sender->transition_db[id_to_write].date,"%s",calendar);
            //get the totoal siz of char array
            int size_of_data = size_of_array(sender->transition_db[id_to_write].date);
            //stripping \n from date
            sender->transition_db[id_to_write].date[size_of_data - 1] = '\0';
            sprintf(sender->transition_db[id_to_write].record ,"You Sent %s A Total Of %d",receiver->name ,token_amount);
            done_written = 1;
        }
    }
    ...
    for(int i=0;i<MAX_RECORD;i++){
        ...
        if(first_cha == '\0'){
            ...
            sprintf(receiver->transition_db[i].date,"%s",calendar);
            int size_of_data = size_of_array(receiver->transition_db[i].date);
            receiver->transition_db[i].date[size_of_data - 1] = '\0';
            ...
            make_int_array(&id_array,receiver->id);
            int biggest_id = get_biggest_int(id_array);
            int id_to_write = get_index_of_id(id_array,biggest_id);
            if(id_to_write==-1){
                printf("Couldn't Save Record!");
            ...
            receiver->transition_db[id_to_write].record_id = (biggest_id+1);
            sprintf(receiver->transition_db[id_to_write].date,"%s",calendar);
            int size_of_data = size_of_array(receiver->transition_db[id_to_write].date);
            receiver->transition_db[id_to_write].date[size_of_data - 1] = '\0';
            sprintf(receiver->transition_db[id_to_write].record ,"You Received A Total Of %d From %s",token_amount,sender->name);
        }
    }
}

// a function to handle the trasnfer of tokens
//0 means failed and 1 means success
int transfer_token() {
    clear_screen();
    ...
    if (l_amount  > 0 && l_amount <= database[g_is_login].token) {
        ...
        for (int i = 0; i < g_total_user; i++) {
            ...
            if (check_email == 1) {
                ...
                if(database[i].suspend == 1){
                    printf("The Account Is Suspended. Therefore , You Cannot Transfar Any Point To That Account");
                    return 0;
                }
                if(i == g_is_login){
        ...
        if(database[i].name[0] != '\0'){
            ...
            for(int j=0;j<MAX_RECORD;j++){
                ...
                if(first_cha != '\0'){
                    ...
                    database[i].transition_db[j].date
                    );
                }
                if(j==MAX_RECORD-1){
                    fputc('\n',file_write);
...
int retrieve_data_from_file() {
    ...
    while((total_field = fscanf(file_read,input_format,
            ...
            while((fscanf(file_read,"{%d:%[^:]:%[^,}]},",
                    ...
                    database[users_from_file].transition_db[record_counter].date))==3){
